...
<div id="tz_pinboard_wrap">
    <div id="tz_pinboard_top">
        <span>
            <?php if (isset($this->namecategory) && !empty($this->namecategory)) { ?>
                <?php foreach ($this->namecategory as $row) {
                    echo $row->title . "   ";
                }?>
            <?php
            } else {
    ...
    <?php if (isset($this->Pins) && !empty($this->Pins) && count($this->Pins) >= (int)$this->limit_pin) { ?>
        ...
        <?php } else{ ?>
            <div id="tz_append">
                <?php if ($this->tz_layout == "ajaxButton") { ?>
                    <a rel="nofollow" id="tz_append_a" class="btn btn-large btn-block"
                       href="#tz_append"><?php echo JText::_("COM_TZ_PINBOARD_ADD_ITEMS"); ?></a>
                    <p class="btn btn-large btn-block"
                       style="display: none"> <?php echo JText::_("COM_TZ_PINBOARD_NO_ITEMS"); ?></p>
                <?php } ?>
            </div>
            <div id="tz_load" style="display: none;">
                <a rel="nofollow"
                   href="<?php echo JURI::root() . 'index.php?option=com_tz_pinboard&view=pinboard&task=add_ajax&page=2&Itemid=' . JRequest::getInt('Itemid'); ?>">
                </a>
            </div>
